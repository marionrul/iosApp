//
//  AddOrdonnanceViewController.swift
//  MyPeka
//
...
class AddOrdonnanceViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, NSFetchedResultsControllerDelegate {
    
    @IBOutlet weak var nomTextField: UITextField!
    @IBOutlet weak var datePicker: UIDatePicker!
    @IBOutlet weak var docteurPicker: UIPickerView!
    
    var ordonnance : OrdonnanceModel?
    ...
    override func viewDidLoad() {
        ...
        do{
            ...
            self.docteurPicker.reloadAllComponents()
        }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "OkAddOrdonnanceSegue"{
            let nom = self.nomTextField.text
            let date = self.datePicker.date
            let indiceDocteur = self.docteurPicker.selectedRow(inComponent: 0)
            self.docteur = personnesFetched.fetchedObjects![indiceDocteur]
            self.ordonnance = OrdonnanceModel(nom: nom!, datePrescrit: date as NSDate, docteur: self.docteur!)
        }
    }
